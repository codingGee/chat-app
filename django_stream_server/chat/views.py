...
def init(request):
    ...
    if 'username' not in body:
        return JsonResponse(status=200, data={'message':'Username is required to join the channel'})
    ...
    try:
        ...
        token = bytes(client.create_token(user_id=member.username)).decode('utf-8')
        return JsonResponse(status=200, data={'username': member.username, 'token': token, 'apiKey': settings.STREAM_API_KEY})
    
    except Member.DoesNotExist:
        member = Member(username=username)
        member.save()
        token = bytes(client.create_token(user_id=username)).decode('utf-8')
